...
export default function Auth({ onLogin }: AuthProps) {
  ...
  const theme = {
    pageBg: isLight ? "linear-gradient(135deg,#e8eeff,#f0f4ff,#e8f0ff)" : "#0b1326",
    pageText: isLight ? "#1a2340" : "#dae2fd",
    cardBg: isLight ? "#ffffff" : "rgba(19,27,46,0.9)",
    cardBorder: isLight ? "rgba(0,0,0,0.08)" : "rgba(255,255,255,0.07)",
    inputBg: isLight ? "#f8faff" : "rgba(255,255,255,0.05)",
    inputBorder: isLight ? "rgba(0,0,0,0.1)" : "rgba(255,255,255,0.1)",
    inputText: isLight ? "#1a2340" : "#dae2fd",
    mutedText: isLight ? "#6b7280" : "#8c909f",
    divider: isLight ? "rgba(0,0,0,0.08)" : "rgba(255,255,255,0.07)",
    dividerText: isLight ? "#9ca3af" : "#8c909f",
    oauthBg: isLight ? "#ffffff" : "rgba(255,255,255,0.04)",
    oauthBorder: isLight ? "rgba(0,0,0,0.08)" : "rgba(255,255,255,0.08)",
    oauthHoverBg: isLight ? "#f8faff" : "rgba(255,255,255,0.08)",
    oauthHoverBorder: isLight ? "rgba(0,0,0,0.12)" : "rgba(255,255,255,0.15)",
    tabHover: isLight ? "#1a2340" : "#dae2fd",
  }
    ...
    <>
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        body { background:${theme.pageBg}; color:${theme.pageText}; font-family:'Inter',sans-serif; }
        .auth-input { width:100%; background:${theme.inputBg}; border:1px solid ${theme.inputBorder}; border-radius:12px; padding:14px 16px 14px 44px; color:${theme.inputText}; font-size:14px; font-family:'Inter',sans-serif; outline:none; transition:all 0.2s; }
        .auth-input:focus { border-color:rgba(59,130,246,0.5); background:rgba(59,130,246,0.05); }
        .auth-input::placeholder { color:${theme.mutedText}; opacity:0.8; }
        .oauth-btn { width:100%; display:flex; align-items:center; justify-content:center; gap:10px; padding:13px; border-radius:12px; background:${theme.oauthBg}; border:1px solid ${theme.oauthBorder}; color:${theme.inputText}; font-size:13px; font-weight:600; cursor:pointer; transition:all 0.2s; font-family:'Inter',sans-serif; }
        .oauth-btn:hover { background:${theme.oauthHoverBg}; border-color:${theme.oauthHoverBorder}; }
        ...
        .tab { flex:1; padding:12px; border:none; background:none; color:${theme.mutedText}; font-size:14px; font-weight:600; cursor:pointer; border-bottom:2px solid transparent; transition:all 0.2s; font-family:'Inter',sans-serif; }
        .tab.active { color:#3B82F6; border-bottom-color:#3B82F6; }
        .tab:hover:not(.active) { color:${theme.tabHover}; }
      ...
      <div style={{ minHeight:"100vh", background:theme.pageBg, display:"flex", alignItems:"center", justifyContent:"center", padding:"24px", position:"relative", overflow:"hidden" }}>
        ...
        <div className="fade-in" style={{ width:"100%", maxWidth:"440px" }}>
          <div style={{ textAlign:"center", marginBottom:"40px" }}>
            ...
            <h1 style={{ fontFamily:"Manrope,sans-serif", fontWeight:800, fontSize:"28px", color:theme.pageText, letterSpacing:"-0.02em" }}>DocMind AI</h1>
            <p style={{ fontSize:"13px", color:theme.mutedText, marginTop:"6px" }}>Your intelligent document assistant</p>
          </div>

          <div style={{ background:theme.cardBg, border:`1px solid ${theme.cardBorder}`, borderRadius:"24px", overflow:"hidden", boxShadow:"0 24px 64px rgba(0,0,0,0.18)" }}>
            <div style={{ display:"flex", borderBottom:`1px solid ${theme.divider}`, padding:"0 8px" }}>
            ...
            <div style={{ padding:"32px" }}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", marginBottom:"24px" }}>
                {[
                  ...
                  <button key={provider} className="oauth-btn" onClick={() => handleOAuth(provider)}>
                    <img src={icon} alt={provider} style={{ width:"18px", height:"18px", filter: !isLight && (provider === "GitHub" || provider === "Apple") ? "invert(1)" : "none" }} />
                    {provider}
                  </button>
                ))}
              </div>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:"12px", marginBottom:"24px" }}>
                ...
                <span style={{ fontSize:"12px", color:theme.dividerText, fontWeight:600 }}>OR CONTINUE WITH EMAIL</span>
                <div style={{ flex:1, height:"1px", background:theme.divider }} />
              </div>
              ...
              {error && (
                <div style={{ background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", borderRadius:"10px", padding:"12px 16px", marginBottom:"16px", display:"flex", alignItems:"center", gap:"10px" }}>
                  ...
                  <span style={{ fontSize:"13px", color:isLight ? "#b91c1c" : "#fca5a5" }}>{error}</span>
                </div>
              )}
              {success && (
                <div style={{ background:"rgba(74,222,128,0.1)", border:"1px solid rgba(74,222,128,0.2)", borderRadius:"10px", padding:"12px 16px", marginBottom:"16px", display:"flex", alignItems:"center", gap:"10px" }}>
                  ...
                  <span style={{ fontSize:"13px", color:isLight ? "#15803d" : "#86efac" }}>{success}</span>
                </div>
              )}
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:"14px", marginBottom:"20px" }}>
                {tab === "signup" && (
                  <div style={{ position:"relative" }}>
                    <div style={{ position:"absolute", left:"14px", top:"50%", transform:"translateY(-50%)", pointerEvents:"none" }}>
                      <Icon name="person" size={18} color={theme.mutedText} />
                    </div>
                ...
                <div style={{ position:"relative" }}>
                  <div style={{ position:"absolute", left:"14px", top:"50%", transform:"translateY(-50%)", pointerEvents:"none" }}>
                    <Icon name="mail" size={18} color={theme.mutedText} />
                  </div>
                ...
                <div style={{ position:"relative" }}>
                  <div style={{ position:"absolute", left:"14px", top:"50%", transform:"translateY(-50%)", pointerEvents:"none" }}>
                    <Icon name="lock" size={18} color={theme.mutedText} />
                  </div>
                  ...
                  <button onClick={() => setShowPassword(p => !p)} style={{ position:"absolute", right:"14px", top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", display:"flex" }}>
                    <Icon name={showPassword ? "visibility_off" : "visibility"} size={18} color={theme.mutedText} />
                  </button>
                </div>
                ...
                {tab === "signup" && (
                  <div style={{ position:"relative" }}>
                    <div style={{ position:"absolute", left:"14px", top:"50%", transform:"translateY(-50%)", pointerEvents:"none" }}>
                      <Icon name="lock_reset" size={18} color={theme.mutedText} />
                    </div>
              ...
              {tab === "signup" && (
                <p style={{ fontSize:"11px", color:theme.mutedText, textAlign:"center", marginTop:"16px", lineHeight:1.6 }}>
          ...
          <p style={{ textAlign:"center", fontSize:"12px", color:theme.mutedText, opacity:0.6, marginTop:"24px" }}>
            DocMind AI · Powered by Claude · Built with love
          </p>
        </div>
      </div>
    </>
